...
import { Synclite, type SyncliteConfig, type SyncStatus } from '@synclite/core'
import { SYNCLITE_KEY } from './plugin.js'

/**
 * Set up Synclite in a root (or layout) component. Provides the instance to all
 * descendants via `useSynclite()`. Call once at the component tree root.
 *
 * Prefer `app.use(createSynclite(config))` for app-wide setup; use this composable
 * when you need Synclite only inside a specific subtree.
 *
 * @example
 * ```ts
 * // RootLayout.vue
 * const db = useSyncliteSetup({ relay: 'wss://relay.example.com' })
 * ```
 */
export function useSyncliteSetup(config: SyncliteConfig): Synclite {
...
/**
 * Access the Synclite instance provided by a parent `useSyncliteSetup()` call
 * or the `createSynclite()` plugin.
 *
 * Throws if called outside a Synclite context.
 */
export function useSynclite(): Synclite {
  const db = inject(SYNCLITE_KEY)
  if (!db) {
    throw new Error(
      'Synclite: useSynclite() was called outside of a Synclite context. ' +
        'Install the plugin with app.use(createSynclite(config)) or call ' +
        'useSyncliteSetup(config) in an ancestor component.',
    )
  }
  return db
}
